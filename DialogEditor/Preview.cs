...
namespace DialogEditor
{
    public partial class Preview : Form
    {
        ...
        public Preview(TreeNode treeNode)
        {
        ...
        private void Preview_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < treeNode.Nodes.Count; i++)
            {
                ...
                //att = treeNode.Nodes[i].Tag as Dialog.AttType;
                //if (att.Pos == "Left")
                //{
                ...
                if (att.Pos == "Left")
                {
                    ...
                    if (att.IsOption)
                    {
                        tbView.Text += att.OptionName1 + (att.OptionName1 == "" ? "" : ":") +
                                       (att.OptionName1 == "" ? "" : (int.Parse(att.OptionAction1) + 1).ToString()) +
                                       "\r\n" +
                                       att.OptionName2 + (att.OptionName2 == "" ? "" : ":") +
                                       (att.OptionName2 == "" ? "" : (int.Parse(att.OptionAction2) + 1).ToString()) +
                                       "\r\n" + att.OptionName3 + (att.OptionName3 == "" ? "" : ":") +
                                       (att.OptionName3 == "" ? "" : (int.Parse(att.OptionAction3) + 1).ToString()) +
                                       "\r\n" + att.OptionName4 + (att.OptionName4 == "" ? "" : ":") +
                                       (att.OptionName4 == "" ? "" : (int.Parse(att.OptionAction4) + 1).ToString()) +
                                       "\r\n\r\n";
                    }
                }
                else
